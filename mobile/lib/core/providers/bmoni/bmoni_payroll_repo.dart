...
class BmoniPayrollRepository implements PayrollRepository {
  ...
  PayrollRunModel _mapRunModel(dynamic json, {required bool isDemo}) {
    ...
    final mappedItems = itemsList.map((i) {
      final cur = Currency.fromCode(i['targetCurrency'] ?? i['target_currency'] ?? 'NGN');
      ...
      final usdAmount = i['usdAmountFormatted'] != null
          ? Money.fromMajorString(i['usdAmountFormatted'].toString(), Currency.usd)
      ...
      return PayrollItemModel(
        ...
        employeeName: i['name'] ?? i['employeeName'] ?? i['employee_name'] ?? '',
        country: i['country'] ?? 'NG',
        targetCurrency: cur,
        destinationStablecoin: stablecoin,
        targetAmount: targetAmount,
        usdAmount: usdAmount,
        exchangeRate: (i['exchangeRate'] as num?)?.toDouble() ?? (i['exchange_rate'] as num?)?.toDouble() ?? 1.0,
    ...
    final totalUsd = data['totalUsdFormatted'] != null
        ? Money.fromMajorString(data['totalUsdFormatted'].toString(), Currency.usd)
    ...
    final totalFeeUsd = data['totalFeeUsdFormatted'] != null
        ? Money.fromMajorString(data['totalFeeUsdFormatted'].toString(), Currency.usd)
    ...
    final savedUsd = data['totalSavedUsdFormatted'] != null
        ? Money.fromMajorString(data['totalSavedUsdFormatted'].toString(), Currency.usd)
        : Money.fromMajorString('330.00', Currency.usd);

    final countries = data['countries'] != null
        ? List<String>.from(data['countries'])
        : (mappedItems.isNotEmpty ? mappedItems.map((i) => i.country).toSet().toList() : ['NG', 'MX']);

    final currencies = data['currencies'] != null
        ? List<String>.from(data['currencies'])
        : (mappedItems.isNotEmpty ? mappedItems.map((i) => i.targetCurrency.code).toSet().toList() : ['NGN', 'MXN']);
    ...
    return PayrollRunModel(
      ...
      employeeCount: data['employeeCount'] ?? (mappedItems.isNotEmpty ? mappedItems.length : 2),
      countries: countries,
      currencies: currencies,
      items: mappedItems,
      status: (data['status'] ?? 'COMPLETED').toString(),
      executedAt: DateTime.tryParse(data['executedAt']?.toString() ?? data['executed_at']?.toString() ?? data['createdAt']?.toString() ?? '') ?? DateTime.now(),
      isDemo: isDemo,
    );
  }
}
