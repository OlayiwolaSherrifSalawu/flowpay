...
class ClientMissionValidator {
  ...
  static MissionValidationResult validate(MissionIntent intent) {
    ...
    if (!supportedCurrencies.contains(intent.triggerCondition.sourceCurrency)) {
      errors.add('Source currency "${intent.triggerCondition.sourceCurrency.code}" is not supported');
    }

    // 3. Source Amount
    final sourceAmount = double.tryParse(intent.triggerCondition.sourceAmount) ?? 0.0;
    if (sourceAmount <= 0) {
    ...
    if (intent.allocations.isEmpty) {
      ...
      return MissionValidationResult(isValid: false, errors: errors, warnings: warnings);
    }
    ...
    for (int i = 0; i < intent.allocations.length; i++) {
      ...
      if (!supportedCurrencies.contains(alloc.targetCurrency)) {
        errors.add('$prefix: Currency "${alloc.targetCurrency.code}" is unsupported');
      }
      ...
      if (alloc.actionType == MissionActionType.transfer &&
          (alloc.recipientIdentifier == null || alloc.recipientIdentifier!.trim().isEmpty)) {
