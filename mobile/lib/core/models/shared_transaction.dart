...
class SharedTransactionModel {
  ...
  final String?
      bmoniReference; // Sanitized public reference (proposalId, txHash) - NEVER secret keys!
  ...
  factory SharedTransactionModel.fromPayrollRun(PayrollRunModel run) {
    ...
    return SharedTransactionModel(
      id: run.runId,
      type: status == TransactionStatus.failed
          ? TransactionType.failure
          : TransactionType.payrollRun,
      title: run.title,
      description:
          'Disbursed ${run.totalUsd.formatted} to ${run.employeeCount} employees across ${run.countries.join(', ')}',
      amount: run.totalUsd,
      ...
      flowpayReference:
          'FP-PAY-${run.runId.length > 8 ? run.runId.substring(run.runId.length - 8) : run.runId}',
      ...
      errorReason: run.failedCount > 0
          ? '${run.failedCount} employee payouts failed'
          : null,
      metadata: {
        'runId': run.runId,
  ...
  factory SharedTransactionModel.fromPayrollItem(
    ...
    final status = isFailed
        ...
        : TransactionStatusX.fromString(item.status);

    return SharedTransactionModel(
      id: 'pay_item_${item.employeeId}_${item.proposalId ?? date.millisecondsSinceEpoch}',
      type:
          isFailed ? TransactionType.failure : TransactionType.employeePayment,
      title: 'Salary: ${item.employeeName}',
      description:
          '${item.targetAmount.formatted} ${item.destinationStablecoin} via ${item.country} rail',
  ...
  factory SharedTransactionModel.fromCardTransaction(
    ...
    final isFailed = tx.status.toUpperCase() == 'DECLINED' ||
        tx.status.toUpperCase() == 'FAILED';
    final status = isFailed
        ? TransactionStatus.failed
        : TransactionStatusX.fromString(tx.status);

    return SharedTransactionModel(
      id: tx.id,
      type:
          isFailed ? TransactionType.failure : TransactionType.cardTransaction,
      ...
      flowpayReference:
          'FP-CRD-${tx.id.length > 6 ? tx.id.substring(tx.id.length - 6) : tx.id}',
      bmoniReference: 'TX-${tx.id}',
      counterparty: tx.merchantName,
      country: 'US',
      errorReason: isFailed
          ? 'Card spend declined (merchant rule or spend ceiling)'
          : null,
      metadata: {
        'cardId': tx.cardId,
  ...
  factory SharedTransactionModel.fromWalletTransaction(
      EmbeddedWalletTransaction tx) {
    ...
    final amountMoney =
        Money.fromMajorString(tx.amount.toStringAsFixed(2), currency);

    return SharedTransactionModel(
      id: tx.id,
      type:
          isFailed ? TransactionType.failure : TransactionType.walletOperation,
      title: tx.title.isNotEmpty ? tx.title : 'Wallet Transfer',
      description: tx.description ??
          (tx.isIncoming
              ? 'Incoming funds on-chain'
              : 'Disbursement from wallet'),
      amount: amountMoney,
      status: status,
      timestamp: tx.createdAt,
      flowpayReference: tx.reference ??
          'FP-WLT-${tx.id.length > 6 ? tx.id.substring(tx.id.length - 6) : tx.id}',
      bmoniReference: tx.id,
      counterparty: tx.counterpartyName ?? 'Smart Wallet',
      country: currency == Currency.ngn
          ? 'NG'
          : (currency == Currency.mxn ? 'MX' : 'US'),
      errorReason: isFailed
          ? 'Smart wallet transfer reverted or failed threshold'
          : null,
      metadata: tx.metadata ??
          {'walletId': tx.walletId, 'direction': tx.direction.name},
    );
  }
  ...
  factory SharedTransactionModel.fromActivity(ActivityModel act) {
    ...
    return SharedTransactionModel(
      ...
      flowpayReference: act.reference,
      bmoniReference: act.reference,
      counterparty: act.metadata?['recipient'] ?? act.metadata?['merchant'],
      country: act.metadata?['rail'] == 'CNGN'
          ? 'NG'
          : (act.metadata?['rail'] == 'MEXe' ? 'MX' : 'US'),
